  ...
  handleSubmit = e => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        Promise.all([payloads.forEach(payload => dispatch(payload))])
          .then(results => {
    ...
    const companyFormItem = keys.map((k, index) => {
      ...
      children.push (
        ...
        >
          ...
          })(<Input addonBefore={selectBefore} addonAfter={selectAfter} placeholder="littleleague" style={{width: '60%', marginRight: 8}} />)}
        </Form.Item>,
        <Form.Item
          label={`What ${index + 1}`}
          required={false}
        ...
        >
          ...
          })(<Input placeholder="e.g. Head Coach (2x Champs!!!)" style={{width: '60%', marginRight: 8}} />)}
        </Form.Item>,
        <Form.Item
          label={`When ${index + 1}`}
          required={false}
